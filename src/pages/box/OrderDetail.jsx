  ...
  return (
    <div className="w-full text-sm">
      <div className="bg-white">
        <h1 className="text-2xl font-bold text-blue-800 py-5 px-5">
          🧾 Chi tiết đơn hàng
        </h1>
      </div>
      ...
      <div className="mx-5 my-5 p-6 bg-white rounded-lg shadow-md">
        ...
        <div className="mb-4 space-y-1">
          <p>
            <span className="font-semibold">Mã đơn:</span> {info.code}
          </p>
          <p>
          ...
          <p>
            <span className="font-semibold">Khách hàng:</span> {info.customer_name || "—"}
          </p>
          <p>
            <span className="font-semibold">Cửa hàng:</span> {info.shop_name || "—"}
          </p>
        </div>
        ...
        <div className="flex justify-end mt-6">
          <div className="w-full sm:w-1/2 md:w-1/3 bg-gray-50 border border-gray-300 rounded p-4 shadow-sm">
            <div className="flex justify-between text-sm mb-1">
              <span>Tạm tính</span>
              <span>{(info.total_price - info.tax).toLocaleString()}₫</span>
            </div>
            <div className="flex justify-between text-sm mb-1">
              <span>Thuế (VAT 8%)</span>
              <span>{Number(info.tax).toLocaleString()}₫</span>
            </div>
            <hr className="my-2" />
            <div className="flex justify-between text-lg font-semibold text-blue-700">
              <span>Tổng cộng</span>
              <span>{Number(info.total_price).toLocaleString()}₫</span>
            </div>
          </div>
        </div>
